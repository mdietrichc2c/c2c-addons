...
class AccountInvoice(orm.Model):
        ...
        for invoice in self.browse(cr, uid, ids, context=context):
            invoice.write({
                'rate_eur': currency_obj._get_conversion_rate(
                    cr, uid, invoice.currency_id, eur, context=context),
                'rate_usd': currency_obj._get_conversion_rate(
                    cr, uid, invoice.currency_id, usd, context=context),
                'amount_untaxed_eur': currency_obj.compute(
                    cr, uid, invoice.currency_id.id, eur_id,
                    invoice.amount_untaxed, context=context),
                'amount_untaxed_usd': currency_obj.compute(
                    cr, uid, invoice.currency_id.id, usd_id,
                    invoice.amount_untaxed, context=context),
            })
    ...
    def copy_data(self, cr, uid, id, default, context):
        ...
        return super(AccountInvoice, self).copy_data(cr, uid, id, default,
                                                    context)
